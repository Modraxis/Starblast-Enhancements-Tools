(function(){
  ...
  let parseURL = async function (url) {
    try {
  ...
  var index = setInterval(function(){
    ...
    if ((data.game_info && music) && (sentence.startsWith(words[0]) && sentence.endsWith(words[1]))) {
      ...
      window.applyMusic = function() {
        music.music.src = '';
        if (localStorage.getItem("ex-enabled") == "true") parseURL(localStorage.getItem("music-external")).then(src => {
          music.music.innerHTML = '';
          let source = document.createElement("source");
          source.setAttribute("src", src);
          music.music.appendChild(source);
          music.music.src = src;
          music.music.load();
          music.music.play();
        });
